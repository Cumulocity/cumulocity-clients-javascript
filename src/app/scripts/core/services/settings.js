(function () {
  ...
  angular
    .module('c8y.core')
    .factory('c8ySettings', c8ySettings);
   ...
   */
  /* @ngInject */
  function c8ySettings(
    $http,
    c8yBase,
    gettextCatalog
  ) {
    var path = 'tenant/options';
    var pathReadOnly = 'tenant/security-options';
    var pathSystem = 'tenant/system/options';
    var config = {
    ...
    return {
      ...
      parseOptionRawValue: parseOptionRawValue,
    ...
    function list(filters) {
      ...
      var onList = c8yBase.cleanListCallback('options', list, _filters);
      var promise = $http.get(url, cfg).then(onList);
    ...
    function parseSettingsResponse(res, defaultValue) {
      var rawValue = _.get(res, 'data.value');
      return parseOptionRawValue(rawValue, defaultValue);
    }

    function parseOptionRawValue(rawValue, defaultValue) {
      var value;
      try {
        value = JSON.parse(rawValue);
      } catch (e) {
        value = !_.isUndefined(rawValue) ? rawValue : defaultValue;
      }
      return value;
    }
